...
| `OPENCLI_SITE_SESSION` | adapter default | Set to `ephemeral` or `persistent` to override `siteSession` metadata for browser-backed adapter commands. `ephemeral` releases the command's isolated tab lease when it finishes; the extension may retain that idle tab as a warm reusable page. `persistent` reuses the site's logical session. Per-command `--site-session` takes precedence. |
